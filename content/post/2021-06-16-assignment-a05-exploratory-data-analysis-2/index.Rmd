...
```{r}
y <- read_csv("energy-and-mining.csv", na = c("n/a", "", "NA")) 
names(y)
```
...
## 5. Access to electricity, rural urban
...
summarise(rural=mean(access_to_electricity_rural_percent_of_rural_population), urban=mean(access_to_electricity_urban_percent_of_urban_population))  %>%
 ...
 pivot_longer(-year, names_to = "444", values_to = "cou")  %>% 
...
ggplot(aes(year, as.factor(444)))+
  geom_bar(stat="identity", width=.3,position="dodge")


#ggplot(aes(year, rainfall, fill=as.factor(year)))
