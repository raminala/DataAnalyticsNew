...
summarise(rural=mean(access_to_electricity_rural_percent_of_rural_population), urban=mean(access_to_electricity_urban_percent_of_urban_population))  %>%
 ...
 ggplot(aes(as.character(year), percentage, fill=as.factor(rural_urban)))+
...
## 6. Oil rents (% of GDP) - Country Ranking
...
y6 <- read_csv("energy-and-mining.csv", na = c("n/a", "", "NA")) %>%
  ...
  ggplot(aes(co2_emissions_from_gaseous_fuel_consumption_percent_of_total,
             energy_imports_net_percent_of_energy_use, size=oil_rents_percent_of_gdp))+ geom_point(color="red")+
  
  
labs(x = "co2_emissions_from_gaseous_fuel_consumption_percent_of_total",
     y = "energy_imports_net_percent_of_energy_use",
     title="Access to different forms of electricity")
